import { Shop } from '../Shop';

export const shops: Shop[] = [
  {
    id: 1,
    name: 'パティスリー　ジョリクレール',
    description:
      '当店は雄大な自然が広がる「北海道・北斗市」にある洋菓子専門店です。パティシエ自慢の季節感・旬感を大事にした創作スイーツを多数取り揃えております。',
    area: '函館市近郊エリア',
    postcode: '049-0162',
    address: '北斗市中央2-1-5',
    openingHoursSpecification: '10:00～20:00 ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'info@hokuto-jolicreer.com',
    url: 'http://www.hokuto-jolicreer.com',
    latitude: '41.821807',
    longitude: '140.652181',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131041912.jpg',
    telephone: '0138-73-0370',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 2,
    name: 'たいやき茶屋　北菓り',
    description:
      '函館朝市内にある、たいやき店。生地の美味しさにこだわり、ハネ付きに仕上げています。定番の甘さ控えめの粒あん等のほか、北海道産のリンゴやじゃがいもを使ったオリジナルのたい焼きが人気です。',
    area: '西部エリア',
    postcode: '040-0063',
    address: '函館市若松町11－10（函館朝市内）',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '水曜日（祝日の場合翌日休）',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.771757',
    longitude: '140.726413',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830040341.jpg',
    telephone: '0138-84-1998',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 3,
    name: 'ラパン・ド・ネージュ',
    description:
      'シェフが一人で全てのケーキを作る小さなお店です。道産素材を中心に、こだわりの食材や季節のおいしい果物など旬を大切にていねいなお菓子を作っています。',
    area: '西部エリア',
    postcode: '040-0064',
    address: '函舘市大手町16-1',
    openingHoursSpecification: '10:00～19:00（日曜・祝日18:00） ',
    closed: '火曜日・月1不定休',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://ameblo.jp/lapin-de-neige/',
    latitude: '41.769118',
    longitude: '140.726799',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180201041808.jpg',
    telephone: '0138-27-5505',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 4,
    name: 'プレミアムロール専門店　ガトー・ルーレ宮前店',
    description:
      '今までにない味と食感を追究して作り上げたロールケーキ。定番の5種と季節商品2種が毎月発売され、イベント月には限定商品等年間30種類以上の品揃えがあります。ｌ',
    area: '中央エリア',
    postcode: '040-0073',
    address: '函館市宮前町25-8メゾン・ド・ソワ1Ｆ',
    openingHoursSpecification: '10:00～18:00 ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.facebook.com/g.roule0518/',
    latitude: '41.789846',
    longitude: '140.738301',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131023018.jpg',
    telephone: '0138-40-9311',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 5,
    name: 'ボン・ガトー・マリス',
    description:
      '気軽にケーキ１つから買いに来ていただけるような親しみやすいお店を目指しています。大切な日の、笑顔になるお手伝いが出来ればと思います。',
    area: '中央エリア',
    postcode: '040-0082',
    address: '函館市白鳥町2-7',
    openingHoursSpecification: '10:00～19:30 ',
    closed: '水曜日（祝日の場合、翌木曜日休み）',
    parking: 'なし',
    eatin: 'undefined',
    email: 'なし',
    url: 'なし',
    latitude: '41.791858',
    longitude: '140.743594',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170119025224.jpg',
    telephone: '0138-42-2477',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 6,
    name: 'はるの餅',
    description:
      'お餅も餡も全て手作り。大門で40年の餅職人が作る懐かしくも新しいお餅をご賞味ください。',
    area: '西部エリア',
    postcode: '040-0035',
    address: '函館市松風町2-5',
    openingHoursSpecification: '8:00～18:30 ',
    closed: '不定休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://haruno-mochi.com/',
    latitude: '41.770114',
    longitude: '140.73216',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170123040703.jpg',
    telephone: '0138-23-3133',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 7,
    name: 'パティスリー　ばら苑',
    description: '誰が食べても一口目に「美味しい」の一言を目指して･･･',
    area: '北部エリア',
    postcode: '041-0851',
    address: '函館市本通2-54-1',
    openingHoursSpecification: '10:00～18:00 ',
    closed: '木曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.802889',
    longitude: '140.766379',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130831083453.jpg',
    telephone: '0138-53-0419',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 8,
    name: 'パティスリー　アキラ',
    description:
      'おいしさのために鮮度と素材にこだわった、確かな技術をもつ小さなフランス菓子の店。',
    area: '中央エリア',
    postcode: '040-0012',
    address: '函館市時任町34-23',
    openingHoursSpecification: '10:30～19:00 ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.78249',
    longitude: '140.757263',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830043314.jpg',
    telephone: '0138-51-1187',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 9,
    name: '欧風菓子　ピラミッド',
    description:
      '急に食べたくなった時も、手ぶらで帰れないアナタも。深夜12時まで営業しています。',
    area: '中央エリア',
    postcode: '040-0011',
    address: '函館市本町1-33　スターパレスビル1Ｆ',
    openingHoursSpecification: '17:00～24:00 ',
    closed: '日曜日・祝日',
    parking: 'なし',
    eatin: 'undefined',
    email: 'なし',
    url: 'なし',
    latitude: '41.787834',
    longitude: '140.749883',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130901022559.jpg',
    telephone: '0138-52-2565',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 10,
    name: '大黒屋菓子店',
    description: '食べて、ホッとするお菓子。食べて、笑顔になるお菓子。',
    area: '函館市近郊エリア',
    postcode: '049-0161',
    address: '北斗市飯生2-8-4',
    openingHoursSpecification: '9:00～19:30 ',
    closed: '日曜日',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.819113',
    longitude: '140.641212',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161207021645.jpg',
    telephone: '0138-73-3152',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 11,
    name:
      'パティスリー　プティ・メルヴィーユ　金森赤レンガ倉庫ＢＡＹはこだて店',
    description:
      '函館の観光名所「赤レンガ倉庫」にある店舗です。1996年9月発売以来、プティ・メルヴィーユの代表作「メルチーズ」や「かぼちゃプリン」をはじめ厳選素材を使ったスイーツをお楽しみ下さい。',
    area: '西部エリア',
    postcode: '040-0065',
    address: '函館市豊川町11-5',
    openingHoursSpecification: '9:30～19:00 ',
    closed: '無休※赤レンガ倉庫の休館日はお休みとなります。',
    parking: '有料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.petite-merveille.jp/',
    latitude: '41.766687',
    longitude: '140.717363',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830043816.jpg',
    telephone: '0138-84-5677',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 12,
    name: 'グルマン　カンタ',
    description: '私たちは伝統と新しいスタイルで美味しさをお届け致します。',
    area: '中央エリア',
    postcode: '040-0062',
    address: '函館市梁川町10-24　SHOP86',
    openingHoursSpecification: '8:00～19:00 ',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'https://www.facebook.com/gourmands.kanta',
    latitude: '41.789729',
    longitude: '140.744584',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130831083546.jpg',
    telephone: '0138-33-5501',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 13,
    name: 'スイーツ　けやき　どら焼のお店',
    description:
      '北海道産の無添加素材を使った、40種類以上のオリジナルどら焼を一つ一つ手造りで製造販売。',
    area: '北部エリア',
    postcode: '041-0807',
    address: '函館市北美原２丁目24-13',
    openingHoursSpecification: '10:00～17:00 ',
    closed: '月曜日、火曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://sweets-keyaki.meisanichiba.jp',
    latitude: '41.830718',
    longitude: '140.747748',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830040913.jpg',
    telephone: '0138-47-4561',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 14,
    name: 'チッチョ　パスティッチョ',
    description:
      'オーナーシェフが現地で学び、食べ歩いたイタリア北部・中部の伝統菓子や郷土菓子をベースにした愛らしいドルチェが並ぶ。ケーキやプリンなどの生菓子の他、クッキーやマカロンなどの焼き菓子も提供しております。',
    area: '北部エリア',
    postcode: '041-0802',
    address: '函館市石川町316-5',
    openingHoursSpecification: '10:00～20:00（日曜日10:00～19:00） ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://cicciopasticcio.jp',
    latitude: '41.825005',
    longitude: '140.741024',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830033931.jpg',
    telephone: '0138-34-7020',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 15,
    name: 'はこだて柳屋',
    description: '和菓子店',
    area: '中央エリア',
    postcode: '040-0075',
    address: '函館市万代町3-13',
    openingHoursSpecification: '8:30～20:00 ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.ikayokan.com',
    latitude: '41.787033',
    longitude: '140.731201',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180201050206.jpg',
    telephone: '0138-42-0989',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 16,
    name: 'カドウフーズ　株式会社　嘉福堂',
    description:
      '北海道の恵まれた食材にこだわった、美味しい手づくりメニューを皆様へ',
    area: '北部エリア',
    postcode: '040-0071',
    address: '函館市追分町1-25',
    openingHoursSpecification: '9:00～17:00 ',
    closed: '日曜日・祝日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://kadoufoods.meisanichiba.jp/',
    latitude: '41.797921',
    longitude: '140.732523',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130831084456.jpg',
    telephone: '0138-62-6077',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 17,
    name: '昭和製菓　株式会社',
    description:
      '伝統の味を引き継ぎ半世紀、北海道原料で作り上げた函館スイーツの美味しさを全国へ！',
    area: '北部エリア',
    postcode: '041-0824',
    address: '函館市西桔梗町589-39',
    openingHoursSpecification: '8:00～17:30 ',
    closed: '土曜・日曜・祝日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.ss-showa.com',
    latitude: '41.829792',
    longitude: '140.721239',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130831083742.jpg',
    telephone: '0138-50-8080',
    created_at: '2019-05-27T06:05:03.000Z',
    updated_at: '2019-05-27T06:05:03.000Z',
  },
  {
    id: 18,
    name: 'ミモザ',
    description:
      '大切な方へのお菓子の贈り物・家族や友達との幸せなひとときにミモザのお菓子を。',
    area: '中央エリア',
    postcode: '042-0942',
    address: '函館市柏木町38-2',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '火曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.789382',
    longitude: '140.771157',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180201045231.jpg',
    telephone: '0138-52-6001',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 19,
    name: '有限会社　銀月',
    description:
      '昭和41年創業の和菓子、お団子の専門店。湯の川名物といえばやっぱり「やきだんご」',
    area: '東央エリア',
    postcode: '042-0932',
    address: '函館市湯川町2丁目22-5',
    openingHoursSpecification: '8:30～17:30 ',
    closed: '不定休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.779875',
    longitude: '140.787265',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130052237.jpg',
    telephone: '0138-57-6504',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 20,
    name: 'キングベーク',
    description:
      '昭和4年創業の老舗ベーカリー。道産小麦やバターなど原材料にこだわり、安心・安全な商品作りがモットー。焼きたてパンやケーキ・プリン等のスイーツを販売する他、併設のカフェスペースで軽食も楽しめる。',
    area: '北部エリア',
    postcode: '041-0813',
    address: '函館市亀田本町7-8',
    openingHoursSpecification: '7:00～21:00 ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.802935',
    longitude: '140.73928',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161206092623.jpg',
    telephone: '0138-45-0963',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 21,
    name: '和創菓ひとひら（株式会社吉田食品）',
    description:
      '平成28年3月の新幹線開業に合わせてオープンしたヨシダのお菓子本店「和創菓ひとひら」です。工場併設店舗ならではの、こだわりのお菓子と店内でお菓子を楽しめるイートインコーナーもあります。',
    area: '北部エリア',
    postcode: '041-0824',
    address: '函館市西桔梗町851-13',
    openingHoursSpecification: '9:00～19:00 ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://o-kashi-yoshida.com/',
    latitude: '41.820114',
    longitude: '140.717078',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131042906.jpg',
    telephone: '0138-83-6668',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 22,
    name: 'スウィーツギャラリー北じま',
    description:
      'スウィーツギャラリー北じまは、変わらぬ味・新しい味など豊富な品揃えでお客様を楽しませます。',
    area: '函館市近郊エリア',
    postcode: '049-0422',
    address: '上磯郡木古内町字本町330-4',
    openingHoursSpecification: '9:00～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://kitajima-pan.com',
    latitude: '41.677153',
    longitude: '140.435218',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161215124158.jpg',
    telephone: '01392-2-2209',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 23,
    name: 'ペイストリー　スナッフルス　金森洋物館店',
    description:
      '北海道産の安全で新鮮な材料を使い、シンプルでわかりやすく新鮮な商品を北海道から全国へ提供し続ける。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函舘市末広町13-9',
    openingHoursSpecification:
      '9:30～19:00※季節によって変わる場合があります。 ',
    closed: '建物に準じます。',
    parking: '無料あり(※金森赤レンガ倉庫駐車場あり)',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.snaffles.jp',
    latitude: '41.766457',
    longitude: '140.716947',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161121040459.png',
    telephone: '0138-27-1240',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 24,
    name: '北洋堂',
    description:
      '桜とお城の松前町の中央商店街「城下通り」登城坂の近くにあり、瓦屋根に漆喰壁と江戸時代の商家を連想させる佇まいのお菓子屋です。代々引き継がれた和菓子を中心に季節のお菓子が並べられ、桜を使ったお菓子もたくさん取り揃えております。',
    area: '函館市近郊エリア',
    postcode: '049-1511',
    address: '松前郡松前町字松城64番地',
    openingHoursSpecification: '8:30～18:00 ',
    closed: '木曜日※さくら祭り期間中は営業いたします。',
    parking: '無料あり',
    eatin: 'あり',
    email: 'hokuyoudou@icloud.com',
    url: 'http://www.hokuyodo.com',
    latitude: '41.428534',
    longitude: '140.10864',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161122051412.gif',
    telephone: '0139-42-2058',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 25,
    name: '(有)龍栄堂',
    description: '函館と共に歩んで87年。当地で皆様に親しまれております。',
    area: '中央エリア',
    postcode: '040-0031',
    address: '函舘市上新川町6-7',
    openingHoursSpecification: '9:00～19:00（日曜日・祝日18:00まで） ',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.780576',
    longitude: '140.736895',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180213104234.jpg',
    telephone: '0138-45-1872',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 26,
    name: 'いなふね菓子舗',
    description:
      'お餅からあられまで、和菓子各種がバラエティに揃う。手づくりのあんこが、甘すぎず美味しい。よもぎを使った和菓子が人気で、特に、よもぎまふぃん、よもぎのべこ餅は好評です。',
    area: '東央エリア',
    postcode: '041-0843',
    address: '函館市花園町37-34',
    openingHoursSpecification: '9:00～19:00 ',
    closed: '日曜日',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.79582',
    longitude: '140.780556',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131044949.jpg',
    telephone: '0138-51-5313',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 27,
    name: '函館　凮月堂',
    description:
      '季節に合わせた旬の素材を使った、お菓子をお客様に提供できる様、心掛けております。',
    area: '北部エリア',
    postcode: '041-0802',
    address: '函館市石川町183-1',
    openingHoursSpecification: '平日 10:00～19:00日・祝 10:00～19:00 ',
    closed: '火曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.838334',
    longitude: '140.740122',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131040029.jpg',
    telephone: '0138-86-7026',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 28,
    name: '菓子工房　花園正家',
    description:
      'ほんものの、心を贈る手造りの店。当社は昭和48年創業の和菓子専門店です。近年は特に、健康第一が叫ばれている時代。当店では道産健康食材を取り入れた、銘菓開発に力を注いでおります。是非ご賞味下されば幸いと存じます。',
    area: '北部エリア',
    postcode: '041-0802',
    address: '函館市港町1丁目33-25',
    openingHoursSpecification: '8:30～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.801629',
    longitude: '140.723074',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130050439.jpg',
    telephone: '0138-45-5828',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 29,
    name: '五島軒本店　スイーツ＆デリカショップAshibino',
    description:
      'レストラン五島軒は明治12年に創業し、函館の歴史と洋食文化を今に伝える老舗として、市民の皆様に親しまれてきました。店内の直営ショップ「Ashibino」は、コース料理のデザートとして発展した洋菓子の数々を取り揃え、お客様をお待ちしております。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函館市末広町4-5',
    openingHoursSpecification: '11:00～20:00 ',
    closed: '月曜日（1月～2月のみ）',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.gotoken.hakodate.jp',
    latitude: '41.763624',
    longitude: '140.715987',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161205041628.jpg',
    telephone: '0138-23-1106',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 30,
    name: '千秋庵総本家',
    description:
      '創業以来152年、材料にこだわり手間と時間をかけた、ていねいな菓子作りの店',
    area: '西部エリア',
    postcode: '040-0043',
    address: '函館市宝来町9-9',
    openingHoursSpecification: '9:00～18:00（季節により変更あり） ',
    closed: '月1回水曜日(月により変更あり)',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://sensyuansohonke.co.jp',
    latitude: '41.761108',
    longitude: '140.719445',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161130115436.jpg',
    telephone: '0138-23-5131',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 31,
    name: '函館国際ホテル　モンモランシー',
    description:
      'ホテルオリジナルの焼き立てパンは、食パンやデニッシュ、菓子パン等の豊富な品揃え。季節感のあるケーキをはじめバースデーケーキ等の予約も承ります。また、ギフトにも最適な焼き菓子も取り揃えています。写真は【キャラメリゼ　360円】',
    area: '西部エリア',
    postcode: '040-0064',
    address: '函館市大手町5-10',
    openingHoursSpecification:
      '7:30～20:00（パン11:00～、ケーキ10:00～無くなり次第終了） ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.hakodate-kokusai.jp',
    latitude: '41.769556',
    longitude: '140.722275',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161216035531.jpg',
    telephone: '0138-23-5151',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 32,
    name: 'パティスリー　ショコラティエ　シュウェットカカオ',
    description:
      '函館五稜郭の近郊にお店を構える道内屈指のチョコレート専門店。ケーキや焼き菓子、アイスクリームなど、たくさんのお菓子を取り揃えて皆様をお迎えします。',
    area: '中央エリア',
    postcode: '040-0015',
    address: '函館市梁川町27-16',
    openingHoursSpecification: '10:00～20:00 ',
    closed: '木曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://chouette-cacao.com/',
    latitude: '41.794328',
    longitude: '140.748513',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131041502.jpg',
    telephone: '0138-33-5766',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 33,
    name: '洋菓子のピーターパン',
    description: '昔ながらのケーキのスタイルを今でも続けています。',
    area: '函館市近郊エリア',
    postcode: '041-1105',
    address: '亀田郡七飯町桜町2-1-12',
    openingHoursSpecification: '8:30～20:00 ',
    closed: '不定休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.901322',
    longitude: '140.688809',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180530104717.jpg',
    telephone: '0138-65-7623　',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 34,
    name: 'Short Berry (ショートベリー)',
    description:
      '安心安全な北海道のおいしい食材を中心に使用して、一つ一つ丁寧に作っています。',
    area: '北部エリア',
    postcode: '041-0801',
    address: '函館市桔梗町418-112',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.83487',
    longitude: '140.735376',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131040850.jpg',
    telephone: '0138-87-0500',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 35,
    name: 'ホテル函館ロイヤルケーキショップ',
    description:
      'ハンドメイドで味わい深いケーキの数々。楽しいひとときの演出に、またお土産としてもご利用ください。',
    area: '西部エリア',
    postcode: '040-0034',
    address: '函館市大森町16-9',
    openingHoursSpecification: '10:00～20:30 ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'あり',
    email: 'royal-h@hakodate.or.jp',
    url: 'なし',
    latitude: '41.769077',
    longitude: '140.733182',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170117045404.jpg',
    telephone: '0138-22-8181',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 36,
    name: '八雲　くら屋',
    description:
      'お菓子を通してお客様と喜びを分かち合うお店。1969年に創業。お菓子を一生懸命、造り販売してまいりました。従業員一同心よりお待ちしております。',
    area: '函館市近郊エリア',
    postcode: '049-3105',
    address: '二海郡八雲町東雲町64-1',
    openingHoursSpecification: '9:30～18:30（日曜日9:00～17:00） ',
    closed: '火曜日(時期により変更有)',
    parking: '無料あり',
    eatin: 'undefined',
    email: 'なし',
    url: 'http://kurayakashiho.co.jp/',
    latitude: '42.251736',
    longitude: '140.277456',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130045600.jpg',
    telephone: '0137-62-3231',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 37,
    name: '五勝手屋本舗',
    description:
      '毎月季節の和生菓子が新しくなり、人生の節目毎にお使い頂ける和菓子を多種用意。',
    area: '函館市近郊エリア',
    postcode: '043-0043',
    address: '桧山郡江差町本町38',
    openingHoursSpecification: '8:00～19:00 ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.gokatteya.co.jp/',
    latitude: '41.864384',
    longitude: '140.125691',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130831075114.jpg',
    telephone: '0139-52-0022',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 38,
    name: 'café and bread TOROIKA　トロイカ洋菓子店',
    description:
      '1977年創業以来、素材にこだわり愛情をこめて全て手作りしております。お気に入りの写真やキャラクターをケーキにして大切な日を記憶に残る日にしませんか？当店では100％オーダーメイドのデコレーションケーキができます。何でもご相談ください！！※写真はイメージです。',
    area: '北部エリア',
    postcode: '041-0802',
    address: '函館市石川町122-40',
    openingHoursSpecification: '8:30～20:00 ',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.833254',
    longitude: '140.747122',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131022355.jpg',
    telephone: '0138-46-8885',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 39,
    name: 'ザ・ランド・オブ・ドリーミング',
    description:
      'ケーキはできるだけ添加物を使わず出来たてをお客様にと思っています。ケーキ、喫茶、誰かと楽しみいっぱいの品をそろえています。',
    area: '中央エリア',
    postcode: '040-0062',
    address: '函館市大縄町24-16',
    openingHoursSpecification: '10:30～18:00 ',
    closed: '土・日・祭日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.783589',
    longitude: '140.732773',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180213104713.jpg',
    telephone: '0138-41-3006',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 40,
    name: '末廣庵',
    description:
      '創業は1934年、ここ木古内で地域の素材などを使い木古内町に根ざした菓子を作り続ける',
    area: '函館市近郊エリア',
    postcode: '049-0422',
    address: '上磯郡木古内町本町237',
    openingHoursSpecification: '9:00～18:00 ',
    closed: '不定休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://koukoumoti.com/',
    latitude: '41.676417',
    longitude: '140.436115',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131043319.jpg',
    telephone: '01392-2-2069',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 41,
    name: 'パティスリー　バイゲツ',
    description: '住宅街にある小さなお店です。',
    area: '北部エリア',
    postcode: '041-0805',
    address: '函館市赤川1-4-16',
    openingHoursSpecification: '10:00～20:00 ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://patisserie-baigetsu.com/',
    latitude: '41.823292',
    longitude: '140.761356',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20130830040617.jpg',
    telephone: '0138-46-7238',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 42,
    name: 'モンドール洋菓子店',
    description:
      '函館中道通りに41年前に開店し地域の皆様に支えられてまいりました小さな店舗です。商品価格は出来るだけ抑え良質な材料と技術で皆様に喜んでいただけるよう努めております。',
    area: '北部エリア',
    postcode: '041-0853',
    address: '函館市中道1丁目24-45',
    openingHoursSpecification: '8:30～18:30 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.80388',
    longitude: '140.759307',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161205050314.jpg',
    telephone: '0138-52-0475',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 43,
    name: 'フランス菓子　ペシェ・ミニョン',
    description:
      'シーズン毎に様変わりする花と緑のパティオから差し込む穏やかな陽の光の中で、心おだやかに、できたてのケーキをお楽しみ下さい。',
    area: '中央エリア',
    postcode: '042-0943',
    address: '函館市乃木町1-2',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.pechemignon.co.jp/',
    latitude: '41.779528',
    longitude: '140.761066',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161121030806.jpg',
    telephone: '0138-31-4301',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 44,
    name: 'パティスリー　イグレック',
    description:
      '東京やフランスで修行した静岡出身の女性オーナーパティシエが作るスイーツのお店。季節のフルーツがどっさりのったフルーツタルトや、写真やキャラクターを使ったフォトケーキは、お誕生日や記念日に人気。',
    area: '函館市近郊エリア',
    postcode: '049-0121',
    address: '北斗市久根別2-3-17',
    openingHoursSpecification: '9:30～20:00 （日・祝は19:00まで） ',
    closed: '水曜日・第3木曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://s.ameblo.jp/patisserie-igrek/',
    latitude: '41.825374',
    longitude: '140.673232',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161130110533.jpg',
    telephone: '0138-84-1365',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 45,
    name: 'スイーツカフェ　カロメリア',
    description:
      '道産素材にこだわり、甘さ控えめに仕上げた笑顔を届けるスイーツ作りをめざしています。',
    area: '中央エリア',
    postcode: '040-0005',
    address: '函館市人見町5-54',
    openingHoursSpecification: '10:00～19:00（日曜日は18:00まで） ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'info@caromelia.com',
    url: 'http://caromelia.com',
    latitude: '41.781585',
    longitude: '140.758044',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180213104011.jpg',
    telephone: '0138-32-5060',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 46,
    name: '函館おたふく堂',
    description:
      '北海道産大豆から出来た、豆乳、おから、豆腐を道産小麦粉、道産グラニュー糖、道南の旬の食材を使用した、豆乳しふぉんケーキ専門店です。他にも、おからボーロ、とうふケーキなどの品揃えをしています。',
    area: '北部エリア',
    postcode: '041-0853',
    address: '函館市中道1-22-5コーポなかの1F',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '火曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'otafukudo@gmail.com',
    url: 'http://hakodate-otafuku.com',
    latitude: '41.802422',
    longitude: '140.757371',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161121052832.JPG',
    telephone: '0138-32-2300',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 47,
    name: '株式会社中村屋',
    description: 'あわび最中をはじめ、和菓子、洋菓子を販売しております。',
    area: '函館市近郊エリア',
    postcode: '049-1511',
    address: '松前郡松前町字松城3番地',
    openingHoursSpecification: '9:00～18:00 ',
    closed: '日曜日',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.428645',
    longitude: '140.11004',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161228105050.jpg',
    telephone: '0139-42-2162',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 48,
    name: '菓匠　一福',
    description:
      '焼き菓子や羊羹、上生菓子からロールケーキまで和洋の商品を取り揃えております。中華まんじゅう、どら焼き、あきあじ最中などが人気です。創業は100年になります。',
    area: '函館市近郊エリア',
    postcode: '049-0281',
    address: '北斗市茂辺地2-5-54',
    openingHoursSpecification: '8:00～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'undefined',
    email: 'なし',
    url: 'なし',
    latitude: '41.764887',
    longitude: '140.60436',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131024153.jpg',
    telephone: '0138-75-2035',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 49,
    name: 'ホームメイドアイスクリーム＆シャーベット冨士冷菓',
    description:
      '1947年創業のアイスクリーム＆シャーベット専門店。北海道の新鮮な牛乳、生クリーム、天然の素材を基本に作られています。容器はシンプルに、素材にこだわったアイスクリームです。',
    area: '西部エリア',
    postcode: '040-0034',
    address: '函館市大森町18-15',
    openingHoursSpecification: '10:00～19:00　※11月～4月は18:00閉店 ',
    closed: '木曜日',
    parking: 'なし',
    eatin: 'あり',
    email: 'ice-cream@fujireika.com',
    url: 'http://www.fujireika.com',
    latitude: '41.769746',
    longitude: '140.734937',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161220043559.jpg',
    telephone: '0138-22-3819',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 50,
    name: 'シュークリーム専門店　ママデコレ',
    description:
      '函館山の麓。観光地ベイエリア近くにあるシュークリームの専門店ママデコレ。お土産にも、旅の途中の食べ歩きにも。おひとついかがですか。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函館市末広町16-10',
    openingHoursSpecification: '10:00～18:00',
    closed: '水曜日',
    parking: 'なし',
    eatin: 'なし',
    email: 'xmycj436@yahoo.co.jp',
    url: 'なし',
    latitude: '41.765238',
    longitude: '140.715842',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161122043036.jpg',
    telephone: '0138-27-4008',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 51,
    name: 'おにぎり・大福　ふくふく',
    description:
      'ふくふくは地場の食材にこだわり、安全・安心・おいしい商品をご提供致します。',
    area: '函館市近郊エリア',
    postcode: '049-0122',
    address: '北斗市東浜2丁目13-21',
    openingHoursSpecification: '8:30～18:00',
    closed: '日曜日・第3月曜日（連休）',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.825193',
    longitude: '140.662463',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161130121255.jpg',
    telephone: '0138-73-2921',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 52,
    name: '山田屋菓子舗',
    description:
      'お店手づくりの和菓子は、一つひとつ丁寧に作っております。かもめ島最中や大福餅など、さまざまな和菓子が揃う。中でもいちご大福は遠方から訪れてくれるファンも多い人気商品です。',
    area: '函館市近郊エリア',
    postcode: '043-0041',
    address: '檜山郡江差町字姥神町79-1',
    openingHoursSpecification: '8:30～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.866142',
    longitude: '140.123394',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161130122956.jpg',
    telephone: '0139-52-0302',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 53,
    name: 'キングスイーツ',
    description:
      '地元の老舗ベーカリー「キングベーク」直営のスイーツ専門店。オリジナルソフトクリームやお土産に最適な商品が並ぶ。人気商品「函館ラスク」は試食が可能な他、チョコレート掛けを見学できる実演スペースも併設。',
    area: '西部エリア',
    postcode: '040-0063',
    address: '函館市若松町12-13JR函館駅構内1階',
    openingHoursSpecification: '8:00～20:00 ',
    closed: '無休',
    parking: '有料あり(※函館駅前広場駐車場の場合)',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.77378',
    longitude: '140.726474',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161206095858.jpg',
    telephone: '0138-22-0303',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 54,
    name: 'アンジェリックヴォヤージュ',
    description:
      '函館の港と夜景が見える小さなお店で、楽しくお菓子を作っています。当店のスペシャル、ショコラヴォヤージュをはじめ、お土産に最適な焼菓子も取り揃えています。ご注文を受けてからお作りするクレープもオススメ！',
    area: '西部エリア',
    postcode: '040-0056',
    address: '函館市弥生町3-11',
    openingHoursSpecification: '10:00～19:00（但し、品切れ次第閉店）',
    closed: '月曜日（祝日の場合、翌日火曜日休み）',
    parking: 'なし',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.angeliquevoyage.com',
    latitude: '41.767904',
    longitude: '140.709118',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161206105206.jpg',
    telephone: '0138-76-7150',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 55,
    name: 'ラッキーピエロ　マリーナ末広店',
    description:
      '金森赤レンガ倉庫群そばの海上レストラン。海の景色がご馳走、ゆったりゆっくり154席。お客様においしく・楽しい・幸せな一時を過ごしていただくことを願っております。※ラッキーピエロは函館市内、近郊13店舗。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函館市末広町14-17',
    openingHoursSpecification: '10:00～24:30（土曜日のみ25:30まで） ',
    closed: '無休',
    parking: '無料あり(※契約駐車場30分無料)',
    eatin: 'あり',
    email: 'なし',
    url: 'http://luckypierrot.jp/',
    latitude: '41.766865',
    longitude: '140.715998',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161206112732.jpg',
    telephone: '0138-27-5000',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 56,
    name: '珈琲焙煎工房　函館美鈴　大門店',
    description:
      '昭和21年、函館駅前大門地区に鈴木商店直営の喫茶店を開業。店名を函館市民から一般公募し、美鈴と命名。美鈴直営店第1号店。',
    area: '西部エリア',
    postcode: '040-0035',
    address: '函館市松風町7-1',
    openingHoursSpecification: '10:00～19:00（焙煎時間10:00～18:00） ',
    closed: '無休',
    parking: 'なし',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.misuzucoffee.com',
    latitude: '41.772226',
    longitude: '140.730155',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180213103632.jpg',
    telephone: '0138-23-7676',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 57,
    name: '御菓子処　ひとつ風',
    description:
      '杉並電停から徒歩1分電車道路沿いにあります。地産地消を心掛け季節を感じて頂ける様な菓子を御用意しております。季節行事の楽しい物もございます。特別注文も気軽に御相談下さい。',
    area: '中央エリア',
    postcode: '040-0003',
    address: '函館市松陰町8-1',
    openingHoursSpecification: '9:00～18:30 ',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.78872',
    longitude: '140.759802',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131030355.jpg',
    telephone: '0138-54-8977',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 58,
    name: '元祖　大沼だんご　沼の家',
    description:
      '明治38年、初代堀口亀吉が「大沼だんご」を製造販売致しました。以来、大沼公園と大沼だんごは100余年の歳月を多くの人々に親しまれてまいりました。',
    area: '函館市近郊エリア',
    postcode: '041-1354',
    address: '亀田郡七飯町大沼町145',
    openingHoursSpecification: '8:00～18:00（売り切れ次第閉店） ',
    closed: '無休',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'なし',
    latitude: '41.981692',
    longitude: '140.67085',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161226115555.jpg',
    telephone: '0138-67-2104',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 59,
    name: '厚沢部菓子工房KURAYA',
    description: 'なし',
    area: '函館市近郊エリア',
    postcode: '043-1114',
    address: '桧山郡厚沢部町本町90-2',
    openingHoursSpecification: '8:00～19:00 ',
    closed: '日曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.917212',
    longitude: '140.224028',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131045514.jpg',
    telephone: '0139-64-3103',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 60,
    name: 'もりまちのパンやさん　たむら',
    description:
      'もりまち国道5号線沿いにある工場兼店舗では、地元のお客様、遠方のお客様に好評いただいております。',
    area: '函館市近郊エリア',
    postcode: '049-2311',
    address: '茅部郡森町字上台町326-91',
    openingHoursSpecification: '7:00～19:00（日・祝18:00まで） ',
    closed: '第2火曜日、小学校運動会の日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'https://www.facebook.com/214577812298228/',
    latitude: '42.096757',
    longitude: '140.571279',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161220040835.jpg',
    telephone: '01374-7-1617',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 61,
    name: 'はこだて甘納豆　石黒商店',
    description:
      '函館唯一の甘納豆製造販売店です。着色料、保存料を使用せず甘さも抑えてますので賞味期限は14日と短めですが11種類それぞれの風味の違いを感じられます。他にゆきうさぎモナカ、花み餅等もございます。',
    area: '北部エリア',
    postcode: '041-0812',
    address: '函館市昭和2-11-2',
    openingHoursSpecification: '9:30～17:30',
    closed: '水曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.amanattou.com/',
    latitude: '41.809765',
    longitude: '140.736077',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161220042306.jpg',
    telephone: '0138-41-0839',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 62,
    name: '函館酪農公社　あいす１１８',
    description:
      '函館牛乳の工場敷地内にあるキャンピングカーの小さなお店、あいす１１８では、牛乳本来の味、栄養をそこなわないように、必要最小限しか手を加えていないソフトクリームやチーズなどの乳製品を直売しています。',
    area: '東央エリア',
    postcode: '042-0912',
    address: '函館市中野町118-17',
    openingHoursSpecification:
      '（4、10月）9:30～17:00（5、6、9月）9:00～17:00（7、8月）9:00～18:00（冬期）10:00～16:00 ',
    closed: '（4～10月）無休（11月～3月）水曜日、年末年始',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.e-milk.co.jp',
    latitude: '41.772608',
    longitude: '140.840485',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161221104907.jpg',
    telephone: '0138-58-4460',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 63,
    name: '菓子舗　喜夢良七飯町本店',
    description: '地元の作物・歴史・文化を基にお菓子創りをしています。',
    area: '函館市近郊エリア',
    postcode: '041-1111',
    address: '亀田郡七飯町本町4ｰ5-20',
    openingHoursSpecification: '8:30～19:00 ',
    closed: '不定休',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.kimurakashiho.com/',
    latitude: '41.896442',
    longitude: '140.691075',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161227104720.jpg',
    telephone: '0138-65-3571',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 64,
    name: '菓子舗　喜夢良若松町菓寮',
    description: '地元の作物・歴史・文化を基にお菓子の創作を目指しています。',
    area: '西部エリア',
    postcode: '040-0063',
    address: '函館市若松町28-14',
    openingHoursSpecification: '10:00～18:00 ',
    closed: '火曜日',
    parking: '無料あり',
    eatin: 'あり',
    email: 'なし',
    url: 'http://www.kimurakashiho.com/',
    latitude: '41.77614',
    longitude: '140.73061',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20161227110540.jpg',
    telephone: '0138-27-2221',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 65,
    name: '焼き菓子の店　morico',
    description:
      '小さなお店で焼き菓子、タルト、スコーン、キッシュなどを焼いています。季節のコンフィチュールもあります。',
    area: '中央エリア',
    postcode: '040-0011',
    address: '函館市本町35-9',
    openingHoursSpecification: '11:30～18:00 ',
    closed: '日・月・火曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'http://kr55lemoncake.jugem.jp/',
    latitude: '41.78609',
    longitude: '140.752591',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180131023618.jpg',
    telephone: '0138-83-5730',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 66,
    name: 'ボーバンベーク',
    description: 'なし',
    area: '函館市近郊エリア',
    postcode: '049-0161',
    address: '北斗市飯生3-4-9',
    openingHoursSpecification: '8:30～18:00（なくなり次第閉店） ',
    closed: '月曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.818204',
    longitude: '140.639552',
    imagePath: 'http://www.hakodate-sweets.com/pgm/img/dummy.jpg',
    telephone: '0138-73-5801',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 67,
    name: 'プレミアムロール専門店　ガトー・ルーレ函館駅店',
    description:
      '今までにない味と食感を追究して作り上げたロールケーキ。定番の5種と季節商品2酒が毎月発売されイベント月には限定商品等年間30種類以上の品揃えがあります。ロールケーキの素材で焼き上げた「函館窯出しドーナツ」「デュオショコラ」等、焼菓子も充実しています。',
    area: '西部エリア',
    postcode: '040-0063',
    address: '函館市若松町12-13JR函館駅構内1階',
    openingHoursSpecification: '8:00～20:00 ',
    closed: '無休',
    parking: '有料あり(※函館駅前広場駐車場の場合)',
    eatin: 'なし',
    email: 'なし',
    url: 'http://www.facebook.com/g.roule0518/',
    latitude: '41.77378',
    longitude: '140.726474',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170104121611.jpg',
    telephone: '0138-84-5488',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 68,
    name: '四季の洋菓子　キャトルセゾン',
    description:
      '自然に囲まれた工房で無添加原料を使用し、昔からあるアンティークなケーキをつくっているお店です。1番人気の『シュークリーム』をはじめ、約15種類のケーキと焼き菓子をご用意しております。',
    area: '函館市近郊エリア',
    postcode: '041-1111',
    address: '亀田郡七飯町本町8-17-17',
    openingHoursSpecification: '10:30～18:30 ',
    closed: '木曜日（※最終木曜日・祝日は営業）',
    parking: '無料あり(※駐車場2台可)',
    eatin: 'なし',
    email: 'なし',
    url: 'http://quatresaisons-7e.com/',
    latitude: '41.899866',
    longitude: '140.69906',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170111113204.png',
    telephone: '0138-84-8414',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 69,
    name: 'お菓子工房　レーヴモンブラン',
    description:
      '道内各地で修行をして夢だった洋菓子店を平成28年10月にオープンしました。ケーキは約20種類、マドレーヌなどの焼き菓子は約15種類ご用意して皆様のお越しをお待ちしております。',
    area: '北部エリア',
    postcode: '041-0822',
    address: '函館市亀田港町63-6',
    openingHoursSpecification: '10:00～19:00 ',
    closed: '火曜日',
    parking: '無料あり',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.813808',
    longitude: '140.716929',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170106103451.jpg',
    telephone: '0138-43-7711',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 70,
    name: '浅野屋',
    description:
      '浅野屋は大正12年（1923年）に創業しました。「江差の繁次郎」や「瓶子岩最中」などお土産の和菓子をはじめ、洋菓子も取り揃えております。',
    area: '函館市近郊エリア',
    postcode: '043-0043',
    address: '檜山郡江差町字本町45',
    openingHoursSpecification: '10:00～19:00',
    closed: '元旦のみ',
    parking: '無料あり(※駐車場1台分)',
    eatin: 'なし',
    email: 'なし',
    url: 'なし',
    latitude: '41.864795',
    longitude: '140.125774',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20170116035539.jpg',
    telephone: '0139-52-0220',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 71,
    name: 'ラッキーピエロ　ベイエリア本店',
    description:
      'ベイエリアの佇まいの中に愛嬌たっぷりのピエロくんがお出迎え。ここがラッキーピエロの１号店です。森の中にいるような店内にあるブランコの席や木馬。きっと幼い頃を想い出し懐かしく楽しいひとときになるでしょう。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函館市末広町23-18',
    openingHoursSpecification: '0138-26-0400',
    closed: '10:00～24:30(土曜日のみ25:30まで)',
    parking: 'なし',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.765716',
    longitude: '140.714781',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025646.jpg',
    telephone: '0138-26-2099',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 72,
    name: 'ラッキーピエロ　港北大前店',
    description:
      '１９５０年代のアメリカを思わせる店。プレスリーが青春だった「ラッキーピエロ　港北大前店」です。アンティークな雰囲気の店内で出逢うのは最高の味！',
    area: '北部エリア',
    postcode: '041-0821',
    address: '函館市港町3丁目18-28',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.8089',
    longitude: '140.716037',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025804.jpg',
    telephone: '0138-42-6550',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 73,
    name: 'ラッキーピエロ　十字街　銀座店',
    description:
      '毎日がハッピーなお店、サンタが函館にやってきた。歴史ある街並みの中にあって、歴風文化賞を受賞。建物の中にはクリスマスグッズの数々や、流れるメロディーもクリスマス一色。２階席への階段を上がるとそこはまるで夢の世界。',
    area: '西部エリア',
    postcode: '040-0053',
    address: '函館市末広町8-11',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: 'なし',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.763716',
    longitude: '140.719231',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025849.jpg',
    telephone: '0138-23-2300',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 74,
    name: 'ラッキーピエロ　松陰店',
    description:
      '街の中にあるジャングルです。アンリ・ルソーの名画、熱帯の原始林をイメージした店内は自然の中にいるような開放感と明るい雰囲気が漂っています。',
    area: '中央エリア',
    postcode: '040-0003',
    address: '函館市松陰町22-6',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.791156',
    longitude: '140.762091',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025924.jpg',
    telephone: '0138-32-1777',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 75,
    name: 'ラッキーピエロ　五稜郭公園前店',
    description:
      'ゆったり５４席の五稜郭公園前店は天使が微笑む店です。広々とした空間は天使達の優しさに包まれ、心が和む時を過ごせることでしょう。ほら、天使のおしゃべりが聞こえてきませんか？',
    area: '中央エリア',
    postcode: '040-0001',
    address: '函館市五稜郭町30-14',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.794314',
    longitude: '140.753082',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025956.jpg',
    telephone: '0138-55-4424',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 76,
    name: 'ラッキーピエロ　人見店',
    description:
      '店内にはたくさんの花・花・花…。天井には素敵なぶどう棚。ラッキーピエロの中で唯一大人気のあんかけ焼きそば＆ラーメンが食べられるお店です。',
    area: '中央エリア',
    postcode: '040-0005',
    address: '函館市人見町24-15',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.781185',
    longitude: '140.760894',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130030030.jpg',
    telephone: '0138-53-7477',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 77,
    name: 'ラッキーピエロ　本町店',
    description:
      'ラッキーピエロの新たな伝説が始まるクリムトハウス。グスタフ・クリムトの絵画と白黒を基調にした店内は、シックな大人の雰囲気となっています。',
    area: '中央エリア',
    postcode: '040-0011',
    address: '函館市本町4-18',
    openingHoursSpecification: '10:00～21:00 ',
    closed: 'なし',
    parking: 'なし',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.788053',
    longitude: '140.75098',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130030055.jpg',
    telephone: '0138-56-4469',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 78,
    name: 'ラッキーピエロ　美原店',
    description:
      '銀幕の映画スターたちが函館に大集合!?色んな映画ポスターが店内の壁に所狭しと並んでいます。青春時代をゆっくりと味わえるようなお店です。',
    area: '北部エリア',
    postcode: '041-0806',
    address: '函館市美原2丁目14-3',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.815501',
    longitude: '140.754331',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130030134.jpg',
    telephone: '0138-46-8988',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 79,
    name: 'ラッキーピエロ　函館駅前店',
    description:
      '今までにない紺色を基調にした店作りに、天井や壁一面にアールデコ調のポスターでおしゃれに店内を演出しています。JR函館駅から一番近いお店です。',
    area: '西部エリア',
    postcode: '040-0063',
    address: '函館市若松町17-12',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり((棒二森屋駐車場OK))',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.771937',
    longitude: '140.728835',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130022605.jpg',
    telephone: '0138-26-8801',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 80,
    name: 'ラッキーピエロ　本通店',
    description:
      '総天然色の食欲満開！映画のワンシーンに登場するようなムーディーな店内で、有名な「ボッティチェリ」をテーマにした魅惑の空間のお店です。',
    area: '北部エリア',
    postcode: '041-0851',
    address: '函館市本通4丁目17-32',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.799156',
    longitude: '140.77574',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130023504.jpg',
    telephone: '0138-53-8080',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 81,
    name: 'ラッキーピエロ　昭和店',
    description:
      '８０年代前半、函館の街を一世風靡した大門の人気スポット「テニスレディ」の復刻版的１軒です。もちろん若い人達にとっても楽しさいっぱいのお店です。',
    area: '北部エリア',
    postcode: '041-0812',
    address: '函館市昭和2丁目12-17',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.810109',
    longitude: '140.735933',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130024521.jpg',
    telephone: '0138-44-6600',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
  {
    id: 82,
    name: 'ラッキーピエロ　戸倉店',
    description:
      '聖堂を思わせる建物の側で風車が風の音を奏でている。壁には額縁に入った１５０枚の写真が飾られ、まさにここはハンバーガー博物館。函館空港から一番近いお店です。',
    area: '東央エリア',
    postcode: '042-0953',
    address: '函館市戸倉町30-1',
    openingHoursSpecification: '10:00～24:30(土曜日のみ 25:30まで) ',
    closed: 'なし',
    parking: '無料あり',
    eatin: 'あり',
    email: 'lucky@lplp.jp',
    url: 'http://www.lplp.jp',
    latitude: '41.784359',
    longitude: '140.807638',
    imagePath:
      'http://www.hakodate-sweets.com/pgm/img/main01-20180130025222.jpg',
    telephone: '0138-59-5888',
    created_at: '2019-05-27T06:05:04.000Z',
    updated_at: '2019-05-27T06:05:04.000Z',
  },
];
